...
const out = mkdtempSync(join(tmpdir(), "pendingsave-"));
const bundle = join(out, "pendingSave.mjs");
execFileSync("npx", ["esbuild", "src/lib/pendingSave.ts", "--bundle", "--format=esm",
                     `--outfile=${bundle}`], { stdio: "pipe", shell: true });
const { PendingSave, LatestWins } = await import(pathToFileURL(bundle).href);
...
function fakeTimers() {
  ...
  return {
    ...
    tick() { const all = [...jobs.values()]; jobs.clear(); for (const f of all) f(); },
...

{
  ...
  check("타이머도 하나만 남는다", clock.pending === 0, String(clock.pending));
}

{
  ...
  check("흘려보낼 게 없으면 false", (await p.flush()) === false);
}

{
  ...
  check("버린 뒤 흘려보내도 아무 일 없다", (await p.flush()) === false);
}
