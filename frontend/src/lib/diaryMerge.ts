/**
 * 두 곳(폰·PC, 탭 둘)에서 같은 날 일기를 고쳤을 때 **둘 다 남기는** 합치기.
 *
 * 서버는 화면이 연 뒤에 다른 곳에서 글이 바뀌었으면 저장을 409 로 돌려보낸다(28차 — 예전에는
...
export function mergeDiaryText(theirs: string, mine: string): string {
