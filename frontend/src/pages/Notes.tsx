...
import { REVEAL_ON_ROW } from "../components/ui/reveal";
...
/** 이름을 바꾸거나 옮길 트리 항목 — 문서이거나 폴더. 폴더면 서버가 다른 규칙(확장자 없음·제 안으로 못 옮김)을 쓴다. */
type TreeItem = { path: string; folder: boolean };
...
/** 새 문서 만들기 — **만들기만** 한다(있으면 서버가 409, 덮지 않는다 — 66차). 화면의 만들기 셋(새 노트·
 *  링크로 만들기·'새 문서 만들어 링크')이 이것 하나를 쓴다. 이미 있을 때 무엇을 할지(알리기·열기)는 부르는 쪽이. */
type Made = { ok: true } | { ok: false; exists: boolean; message: string };
async function makeDoc(path: string, content: string): Promise<Made> {
  try {
    await api.noteCreate(path, content);
    return { ok: true };
  } catch (e) {
    return { ok: false, exists: isConflict(e), message: e instanceof Error ? e.message : "문서 생성 실패" };
  }
}
...
export function Notes() {
  ...
  const [renameFor, setRenameFor] = useState<TreeItem | null>(null);
  const [renameName, setRenameName] = useState("");
  const [moveFor, setMoveFor] = useState<TreeItem | null>(null);
  ...
  const createNote = async () => {
    ...
    // 만들기만 한다(makeDoc) — 위 확인은 이 화면의 목록이라 낡았을 수 있다(다른 기기가 방금 만든 것).
    // 서버가 거절하면(같은 이름 등) 열지 않는다 — 없는 문서를 열어 404 를 또 띄운다
    const made = await makeDoc(path, isMd ? `# ${name.replace(/\.[^.]+$/, "")}\n\n` : "");
    if (!made.ok) {
  ...
  const openByTitle = useCallback(
    (title: string, create = true, from: string | null = null) => {
      ...
      void (async () => {
        ...
        const path = `${title}.md`;
        const made = await makeDoc(path, `# ${title}\n\n`);
        if (!made.ok && !made.exists) {
          toast.error(made.message);
          return;
        }
        // 만들었거나 이미 있다 — 이미 있으면 덮지 않고 그것을 연다
        await reloadTree();
        openNote(path);
      })();
    },
            ...
            {!isPinned(child.path) && (
              <>
                <a href={api.noteArchiveUrl(child.path)} download
                  ...
                  // 누를 자리는 … 단추와 같은 28px(붙은 단추가 작으면 휴대폰에서 옆 것이 눌린다).
                  // 보이는 때의 까닭은 REVEAL_ON_ROW.
                  className={`grid h-7 w-7 shrink-0 place-items-center rounded text-fg-muted transition-opacity hover:text-accent ${REVEAL_ON_ROW}`}
  ...
  return (
    <Shell title="문서" actions={actions}>
      <ThreePane storageKey="notes.panes.v1" showDetail={!!current} fixedLabel="문서 트리" sideLabel="미리보기" centerLabel="편집">
        {/* 트리 */}
